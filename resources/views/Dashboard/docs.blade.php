...
<html>
  ...
  <body>
    ...
    <img src="{{$data->setting->paraf}}" style="width:100px;position:absolute; bottom:8px; margin-left:70px;">
    <p style="position:absolute; bottom:-25px; font-size:10px">Dokumen ini telah disetujui dan ditanda tangani secara digital oleh {{$data->setting->signer_name}}  pada platform SIGNaja.com </p>
    @if($i == $data->count && $data->is_signature == "yes")
    <p style="page-break-after: always;"></p>
    @endif
    @endfor
    <br>
    <p>Dokumen dengan judul <span>{{$data->title}}</span> dengan berjumlah {{$data->count}} halaman yang diupload oleh: <br />
    <span>Nama: {{$data->signee->name}}</span><br />
    <span>Email: {{$data->signee->email}}</span><br />
    <span>Tanggal Pengajuan: {{$data->created_at}}</span><br />
